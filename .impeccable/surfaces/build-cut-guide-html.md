...
related_targets: ["src/dass/build_guide.py","web/render/materials.mjs"]
---

Scope: `build/cut-guide.html` and `build/how-its-going.html`, an offline screen
and print drawing set plus its companion field-notes page, generated by
`src/dass/build_guide.py`.
...
assembly steps. Sheets run A-200 timber, A-300 raspont, and A-400 units. The
companion page carries supplied workshop photos in build order, beginning with
the saw setup for beam cuts.

Direction: the Swedish construction drawing set that `docs/original-drawing/drawing-sides.png` comes
...
photoreal renderer, shared through `web/render/materials.mjs`.
...
sideways but the document never does, at any width. The field-notes page keeps
the same drawing-set layout, omits the masthead metadata table, and serves
responsive optimized copies of its photos.
