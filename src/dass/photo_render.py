...
texture atlas from ``web/media/textures/plywood_diff_4k.jpg``, then hands them all to
``web/render/render.mjs``, which lights and photographs the model in headless
Chromium (three.js, sky IBL, soft sun shadows).

    uv run render-photo                        # every view
    uv run render-photo --views open-hero      # one view, fast
    uv run render-photo --width 2400 --height 1800
...
from . import model as dass

ROOT = Path(__file__).resolve().parents[2]

# How far the open variant opens. Named because the dev server exports the same
# two GLBs on its own and has to pose them the same way.
DOOR_ANGLE = 105.0
ROOF_LIFT_ANGLE = 25.0
